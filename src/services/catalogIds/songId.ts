...
/**
 * Maps between the mobile catalog's track ids and the server's `song_id`s.
 * The backend keys songs by a DETERMINISTIC uuid v5 —
 ...
 * and `Track.trackNumber` is `n`, so we can compute the same uuid client-side,
 * and build a reverse map to resolve server ids back to playable `Track`s.
 */
 ...
 * this uuid, as song endpoints key by `songUuid`) must convert through here.
 */
